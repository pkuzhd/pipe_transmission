...
# def rgb2yCbCr_matrix(rgb, device):
...
#     yuv = torch.zeros_like(rgb)
#     yuv = torch.zeros((1,3),device=device)
#     matrix = torch.tensor([[0.1826 , -0.1006 , 0.4392],[0.6142 , -0.3386 , -0.3989] , [0.062 , 0.4392 , -0.0403]])
#     matrix_2 = torch.tensor([[16],[128],[128]])
    
#     matrix = matrix.expand()
    
#     yuv = matrix_2 - matrix@rgb
...
# Read input image for each camera
    cam_index = int(filename)
    print("tesx",cam_index)
    file_path = dataset_path+ "/" + filename + ".tif"
    #test_path = "F:\\Projects\\sphere-stereo-master\\instr\\test"
    image = cv2.imread(file_path, cv2.IMREAD_UNCHANGED)
    #cv2.imwrite(test_path + "\\" + "test write "+str(cam_index) + "in" + filename+".png" , image)
    #print("[util image]" + str(cam_index),image )
    
    # Type and innapropriate file handling
    if image is not None:
        if image.shape == (calibrations[cam_index].original_resolution[1], calibrations[cam_index].original_resolution[0], 3):
    ...
    if valid_frame:
        ...
        if cam_index in references_indices:
            print("[test]",cam_index in references_indices)
            image_to_stitch = cv2.resize(image, tuple(rgb_to_stitch_resolution), cv2.INTER_LANCZOS4)
            
            images_to_stitch.append(image_to_stitch)
        # Resize for matching and distance estimation
        image_to_match = cv2.resize(image, tuple(matching_resolution), cv2.INTER_LANCZOS4)
...
def evaluate_rgbd_panorama(rgbd_panoramas, filename, dataset_path, bad_px_ratio_thresholds, panorama_resolution):
    ...
    try:
        ...
        if(gt_rgb is not None and gt_rgb is not None 
            ...
            gt_rgb = cv2.resize(gt_rgb, tuple(panorama_resolution), cv2.INTER_CUBIC)
            gt_distance = cv2.resize(gt_distance, tuple(panorama_resolution), cv2.INTER_CUBIC)
