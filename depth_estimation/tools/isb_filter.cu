    ...
    uchar3* guideOut, float* costOut, int rowsOut, int colsOut, float varInvI)
{
    ...
    if(indexOut < colsOut * rowsOut)
    {
        ...
        for(int z(0); z < CANDIDATE_COUNT; z++)
        {
            ...
            if (costOut[z * rowsOut * colsOut + y * colsOut + x] == 0)
                costOut[z * rowsOut * colsOut + y * colsOut + x] = currentCost;//-1.0;
        }    
    }
}
...
//         weightSum4 = 1.f / weightSum4;
		...
		// Guide upsampling
        guideInOut[y2 * colsInOut + x2] = float3Touchar3(inSumGuide1 / weightSum1);
        if(setPlusY)
            guideInOut[plusUp.y * colsInOut + x2] = float3Touchar3(inSumGuide2 / weightSum2);
        if(setPlusX)
            guideInOut[y2 * colsInOut + plusUp.x] = float3Touchar3(inSumGuide3 / weightSum3);
        if(setPlusY && setPlusX)
            guideInOut[plusUp.y * colsInOut + plusUp.x] = float3Touchar3(inSumGuide4 / weightSum4);
        ...
        for(int z(0); z < CANDIDATE_COUNT; z++)
        {
            ...
            float neighboursCostDown1[9] = {
                neighboursCostDown[0][0],
                neighboursCostDown[0][1],
                neighboursCostDown[1][0],
                neighboursCostDown[0][2],
                neighboursCostDown[1][1],
                neighboursCostDown[1][2],
                neighboursCostDown[2][0],
                neighboursCostDown[2][1],
                neighboursCostDown[2][2],
            };

            float neighboursCostDown2[6] = {
                neighboursCostDown[1][1],
                neighboursCostDown[2][1],
                neighboursCostDown[1][0],
                neighboursCostDown[2][0],
            ...
            float current_cost;
            if (costInOut[z * rowsInOut * colsInOut + y2 * colsInOut + x2] < 0) {
                current_cost = costInOut[z * rowsInOut * colsInOut + y2 * colsInOut + x2];
                costInOut[z * rowsInOut * colsInOut + y2 * colsInOut + x2] = 0;
//                 weightSum1 = 1.f / weightSum1;
                weightSum1 -= weightUp;
//                 weightSum1 = 1.f / weightSum1;

                //costInOut[z * rowsInOut * colsInOut + y2 * colsInOut + x2] = weightUp * costInOut[z * rowsInOut * colsInOut + y2 * colsInOut + x2];
                for(int i(0); i < 9; i++) {
                    if (neighboursCostDown1[i] < 0.f) {
//                         weightSum1 = 1.f / weightSum1;
                        weightSum1 -= weights1[i];
//                         weightSum1 = 1.f / weightSum1;
                        continue;
                    }
                    costInOut[z * rowsInOut * colsInOut + y2 * colsInOut + x2] += weights1[i] * neighboursCostDown1[i];
                }
                if (costInOut[z * rowsInOut * colsInOut + y2 * colsInOut + x2] * weightSum1 < 1e-9)
                    costInOut[z * rowsInOut * colsInOut + y2 * colsInOut + x2] = current_cost;
                else
                    costInOut[z * rowsInOut * colsInOut + y2 * colsInOut + x2] /= weightSum1;
            ...
            if (costInOut[z * rowsInOut * colsInOut + plusUp.y * colsInOut + x2] < 0) {
                current_cost = costInOut[z * rowsInOut * colsInOut + plusUp.y * colsInOut + x2];
                costInOut[z * rowsInOut * colsInOut + plusUp.y * colsInOut + x2] = 0;
//                 weightSum2 = 1.f / weightSum2;
                weightSum2 -= weightUp;
//                 weightSum2 = 1.f / weightSum2;
                ...
                if(setPlusY) {
                    for(int i(0); i < 6; i++) {
                        if (neighboursCostDown2[i] < 0.f) {
//                             weightSum2 = 1.f / weightSum2;
                            weightSum2 -= weights2[i];
//                             weightSum2 = 1.f / weightSum2;
                            continue;
                        }
                        costInOut[z * rowsInOut * colsInOut + plusUp.y * colsInOut + x2] += weights2[i] * neighboursCostDown2[i];
                    }
                    if (costInOut[z * rowsInOut * colsInOut + plusUp.y * colsInOut + x2] * weightSum2 < 1e-9)
                        costInOut[z * rowsInOut * colsInOut + plusUp.y * colsInOut + x2] = current_cost;
                    else
                        costInOut[z * rowsInOut * colsInOut + plusUp.y * colsInOut + x2] /= weightSum2;
...
//                             ) * weightSum2;
            ...
            if (costInOut[z * rowsInOut * colsInOut + y2 * colsInOut + plusUp.x] < 0) {
                current_cost = costInOut[z * rowsInOut * colsInOut + y2 * colsInOut + plusUp.x];
                costInOut[z * rowsInOut * colsInOut + y2 * colsInOut + plusUp.x] = 0;
//                 weightSum3 = 1.f / weightSum3;
                weightSum3 -= weightUp;
//                 weightSum3 = 1.f / weightSum3;
                ...
                if(setPlusX) {
                    for(int i(0); i < 6; i++) {
                        if (neighboursCostDown3[i] < 0.f) {
//                             weightSum3 = 1.f / weightSum3;
                            weightSum3 -= weights3[i];
//                             weightSum3 = 1.f / weightSum3;
                            continue;
                        }
                        costInOut[z * rowsInOut * colsInOut + y2 * colsInOut + plusUp.x] += weights3[i] * neighboursCostDown3[i];
                    }
                    if (costInOut[z * rowsInOut * colsInOut + y2 * colsInOut + plusUp.x] * weightSum3 < 1e-9)
                        costInOut[z * rowsInOut * colsInOut + y2 * colsInOut + plusUp.x] = current_cost;
                    else
                        costInOut[z * rowsInOut * colsInOut + y2 * colsInOut + plusUp.x] /= weightSum3;
...
//                         ) * weightSum3;
            ...
            if (costInOut[z * rowsInOut * colsInOut + plusUp.y * colsInOut + plusUp.x] < 0) {
                current_cost = costInOut[z * rowsInOut * colsInOut + plusUp.y * colsInOut + plusUp.x];
                costInOut[z * rowsInOut * colsInOut + plusUp.y * colsInOut + plusUp.x] = 0;
//                 weightSum4 = 1.f / weightSum4;
                weightSum4 -= weightUp;
//                 weightSum4 = 1.f / weightSum4;
                ...
                if(setPlusY && setPlusX) {
                    for(int i(0); i < 4; i++) {
                        if (neighboursCostDown4[i] < 0.f) {
//                             weightSum4 = 1.f / weightSum4;
                            weightSum4 -= weights4[i];
//                             weightSum4 = 1.f / weightSum4;
                            continue;
                        }
                        costInOut[z * rowsInOut * colsInOut + plusUp.y * colsInOut + plusUp.x] += weights4[i] * neighboursCostDown4[i];
                    }
                    if (costInOut[z * rowsInOut * colsInOut + plusUp.y * colsInOut + plusUp.x] * weightSum4 < 1e-9)
                        costInOut[z * rowsInOut * colsInOut + plusUp.y * colsInOut + plusUp.x] = current_cost;
                    else
                        costInOut[z * rowsInOut * colsInOut + plusUp.y * colsInOut + plusUp.x] /= weightSum4;
