...
class DTU_Test_Set(Dataset):
    ...
    def __getitem__(self, index):
        ...
        print("! get_item_ img/cam type: ", len(images), len(cams), images[0].shape)
        
        # for i in range(5):
        #     io.write_cam_dtu(f"/home/wph/pipe_transmission/depth_estimation/FastMVSNet/cam_para/before_build_para{i}.txt", cams[i])
        ...
        print("! get_item_ scale img type: ", scaled_input_images[0].dtype, scaled_input_images[0].shape)

        # crop to fit network
        # croped_images, croped_cams, ref_depth = crop_dtu_input(scaled_input_images, scaled_input_cams,                              height=self.height, width=self.width,
        # base_image_size=self.base_image_size, depth_image=ref_depth)
        
        croped_images, croped_cams = crop_dtu_input(scaled_input_images, scaled_input_cams,height=self.height, width=self.width, base_image_size=self.base_image_size, depth_image=None)
        # ref_image = croped_images[0].copy()

        # for i in range(5):
        #     io.write_cam_dtu(f"/home/wph/pipe_transmission/depth_estimation/FastMVSNet/cam_para/build_para{i}.txt", croped_cams[i])

        print("! get_item_ crop img type: ", croped_images[0].dtype, croped_images[0].shape)
        ...
        print(f"! dataset: cam_params_list {cam_params_list.shape}")
